    ...
    private class ClientTask extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... msgs) {
            try {
                ...
                Log.i(TAG, "Remote Port - " + remotePort);

                Socket socket = new Socket(InetAddress.getByAddress(new byte[]{10, 0, 2, 2}),
                        Integer.parseInt(remotePort));

                Log.i(TAG, "Socket created " + socket.getInetAddress().toString());
                if(socket!=null)
                    Log.i(TAG, "Created socket at Port - " + remotePort);
                else Log.i(TAG, "Not able to create socket - Connection = NULL") ;
                ObjectOutputStream outgoing = new ObjectOutputStream(socket.getOutputStream());
                if (outgoing != null) Log.i(TAG, "Output established ");
                else Log.i(TAG, "Failed to establish output stream!");

                String msgToSend = msgs[0];
                outgoing.writeObject(msgToSend);
                outgoing.flush();
                Log.i(TAG, "Message sent from client " + socket.getLocalSocketAddress() + " -- " + msgToSend);
                Log.i(TAG, "Closing Streams and socket from Client end. ");
                outgoing.close();
                socket.close();
